...
"<script>"
"let rows=[];"
"const MC={idle:'#444',scanning:'#3b82f6',devices:'#06b6d4',attacking:'#ef4444',strike:'#f97316',hunt:'#eab308',god:'#d946ef',stopping:'#eab308'};"
...
"<td class='nm' onclick='showDev(${n.i})'>${esc(n.ssid)}${n.prot?` <span class='lock'>PMF</span>`:''}${n.safe?` <span class='safe'>SAFE</span>`:''}</td>"
...
"function attack(){let ids=[...document.querySelectorAll('#list tbody input:checked')].map(c=>c.dataset.i);if(!ids.length)return;"
"let prot=ids.filter(i=>rows[i]&&rows[i].prot).length;"
"let note='Attacks run non-stop until you tap Stop. Your phone disconnects while they run; Stop retries until it lands.';"
...
"let sent=document.getElementById('s'+n.i);if(sent)sent.textContent=n.tx||'';"
"let devc=document.getElementById('v'+n.i);if(devc)devc.textContent=n.clients;})}).catch(e=>{})}"
